...
def droite(point1,point2):
    log.debug("droite(%d,%d)",point1,point2)
...
def choixPos(nEquipe):
        ...
        if not (nEquipe == 2 or nEquipe == 1):
                log.error("Equipe %d impossible",nEquipe)
...
class jeu :
        ...
        def interception(self,joueur1,joueur2):
                pos1 = joueur1.pos
                pos2 = joueur2.pos
                log.info("Interception entre (%d,%d) et (%d,%d)",pos1[0],pos1[1],pos2[0],pos2[1] )
        ...
        def tirAvant(self,pos):
                ...
                if self.porteur:
                        if self.nobodyFront(pos) and self.front(pos) and self.jeu.matrice[pos[0]][pos[1]].nEquipe == 3: 
                            ...
                            log.error("La case choisie n'est pas en avant du joueur %d", self.numero)
