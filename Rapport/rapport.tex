...
Autour du fichier central, \textbf{projet.py}, qui contient le coeur du jeu, nous avons ajout\'e de nombreux fichiers secondaires : plusieurs fichiers pour r\'ealiser les tests, un pour l'interface graphique, un autre pour la gestion des queues et un dernier pour lancer le jeu. Nous voulions assurer une couverture de code maximale.
...
Pour g\'erer le partage du code de façon efficace, nous nous sommes servis de Github, une plate-forme permettant l'\'echange de fichiers. L'avantage de Github est qu'il permet à chacun de modifier librement le code fait par les autres sans que cela affecte la totalit\'e. De cette facon, la coordination entre nous s'est retrouv\'ee consid\'erablement simplifi\'ee car Github se charge d'int\'egrer automatiquement les nouveaut\'es et de synchroniser les fichiers. En cas de conflit, il le signale pour que l'utilisateur le r\`egle manuellement. Pour communiquer, nous avons bien \'evidemment \'echang\'e des mails, mais l'utilisation d'un tchat nous a vite paru indispensable, c'est pourquoi nous nous sommes tourn\'es vers Slack. 
...
Notre espace Trello \'etait constitu\'e : d'une premi\`ere liste montrant les bugs \`a r\'esoudre, d'une deuxi\`eme indiquant ce qui restait \`a faire,  d'une troisi\`eme indiquant ce qui \'etait en cours et d'une derni\`ere affichant ce qui avait \'et\'e fait.
...
Les diff\'erentes classes et m\'ethodes \'etaient imbriqu\'ees les unes dans les autres. Par cons\'equent, tout test unitaire \'etait impossible (ou presque). En premier lieu, nous avons donc pleinement mis \`a profit le concept de mock object.
...
Depuis que l'interface graphique a \'et\'e int\'egr\'ee, il ne nous est plus possible de lancer les tests. En effet, nous lancions ces derniers depuis la version de notre jeu avec interface utilisateur sur la console. Bien entendu, nous avons bien v\'erifi\'e que les fonctions r\'epondaient au cahier des charges avant de finalement les int\'egrer dans l'interface.


\subsection{Interprétation des règles}
Certaines règles sur la résolution d'évènements n'étaient pas précisées, nous avons donc du interpréter le jeu, apporter des réponses cohérentes pour résoudre ces actions sans fausser le jeu. Cependant, nous nous sommes rendus compte que nous avions mal interprété un point de d\'etail des r\`egles. En effet, l'utilisation de telle ou telle carte \textit{forme}, qui participe \`a la r\'esolution des actions, doit \^etre un choix du joueur et non pas le fait du hasard. Contraint par le temps, nous n'avons pas pu corriger cette interprétation qui selon nous n'empêche pas le joueur de jouer et ne fait perdre aucune saveur au jeu.
...
Notre jeu est op\'erationnel, il est possible d'y jouer \`a deux sur une m\^eme machine. Les fonctionnalités principales ont \'et\'e testées plusieurs fois dans des cas distincts et semblent satisfaisantes. On peut enti\`erement jouer \`a la souris gr\^ace l'interface graphique. Il y a une divergence par rapport à ce qui \'etait pr\'evu au d\'epart puisque l'intelligence artificielle n'a finalement pas vu le jour, mais son absence a \'et\'e compensée par une meilleure interface graphique. Cela mis \`a part, le r\'esultat final nous para\^it en accord avec ce que nous avions annonc\'e au d\'ebut.
