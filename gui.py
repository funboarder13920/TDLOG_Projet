...
class askInter(qtc.QThread):
    # Lorsqu'une interception de passe est possible, une boite de dialogue
    # s'ouvre pour demander si le joueur souhaite intercepter
...
class depThread(qtc.QThread):
    # Renvoie la position d'un joueur et celle d'une case de la grille au
    # programme principal
...
class choixThread(qtc.QThread):
    # Récupère le choix de position des équipes et le renvoie au fichier
    # principal
...
class buttonPos(qtg.QPushButton):
    # Class button modifiée pour renvoyer la position lorsque l'on clique sur
    # un bouton
...
class gui1(qtg.QWidget):
    ...
    def showEquipe(self, nEquipe):
        # Changement de l'affichage lorsque c'est au tour de l'équipe suivante
        # de jouer
    ...
    def showPlayerInfo(self, pos1):
        # Affiche des informations sur le joueur sélectionné au dessus du
        # plateau
        try:
    ...
    def endChoixTrue(self):
        # Permet de quitter proprement la boucle principale lorsqu'un joueur a
        # terminé de placer ses joueurs
    ...
    def send(self, button, nJoueur, nEquipe):
        ...
        # Fais passer la boucle à l'étape suivante: attendre une position sur
        # la grille
